    ...
    return `
     <tr>
        <td class="py-4 md:w-[384px]">
          <div class="flex items-center gap-4">
            <a href="#" class="flex items-center aspect-square w-10 h-10 shrink-0">
              <img class="h-80 w-80 max-h-full dark:hidden" src="${item.image}" alt="${item.title} image" />
              <img class="hidden h-80 w-80 max-h-full dark:block" src="${item.image}" alt="${item.title} image" />
            </a>
        ...
        <td class="p-4 text-base font-normal text-gray-900 dark:text-white">
          <button class="cursor-pointer" onclick="removeItemFromCart(${item.id})">Remove</button>
        </td>
 ...
 const cartHTML = remainingCartItem.map(item => {
    return `
     <tr>
                  ...
                  <td class="p-4 text-base font-normal text-gray-900 dark:text-white"><buton class="courosr-pointer" onclick="removeItemFromCart(${item.id})" >Remove</buton></td>
